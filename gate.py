...
def witcher():
    ...
    while True:
        os.system("clear")
        witcher_banner = pfgt.figlet_format("witcher", font="slant")
        ...
        try:
            for target in range(1, target_port_witcher):
                socket_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                final_result = socket_sock.connect_ex((target_address_witcher, target))
...
def banner_grabber():
    ...
    while True:
        ...
        try:
            ...
            time_start = dtt.now()
            bannergrab_banner = pfgt.figlet_format("Banner Grabber", font="slant")
