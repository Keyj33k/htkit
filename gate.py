...
class HunterToolkit:
    ...
    def menu():
        ...
        print(f"{r}[{w}1{r}]{w} Witcher {r}>>{w} A simple port scanner.")
        print(f"{r}[{w}2{r}]{w} MD5Crypt {r}>>{w} MD5 encryption.")
        print(f"{r}[{w}3{r}]{w} WhoisForIP {r}>>{w} Get informations about an ip address.")
        print(f"{r}[{w}4{r}]{w} BannerGrabber {r}>>{w} Get service behind a port.")
        print(f"{r}[{w}5{r}]{w} B64Crypt {r}>>{w} En- and Decryption used base64.")
        print(f"{r}[{w}6{r}]{w} PhoneStalk {r}>>{w} Get informations about a phonenumber")
        print(f"{r}[{w}7{r}]{w} SubdomainScanner {r}>>{w} Get subdomains from any url.")
        print(f"{r}[{w}8{r}]{w} Whoami {r}>>{w} Get infos like current ip address etc.")
        print(f"{r}[{w}9{r}]{w} MySystem {r}>>{w} Get infos like cpu usage, disk usage etc.")
        print(f"{r}[{w}10{r}]{w} GetIPfromURL {r}>>{w} Get the ip address of any url.")
        print(f"{r}[{w}11{r}]{w} PassGen {r}>>{w} Generate a random password.")
        print(f"{r}[{w}12{r}]{w} WhoisForURL {r}>>{w} Whois lookup for URL.")
        print(f"{r}[{w}13{r}]{w} GetHeader {r}>>{w} Get header from url.")
        print(f"{r}[{w}14{r}]{w} LinkCollector {r}>>{w} Get links from any website.")
        print(f"{r}[{w}15{r}]{w} Ping {r}>>{w} Check remote host availability.")
        print(f"{r}[{w}16{r}]{w} IPSweep {r}>>{w} Check host range for active devices.")
